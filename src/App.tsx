...
import robotUrl from './public/robot.png';
...
export default function App() {
  ...
  const [apiKlicVisibility, setApiKlicVisibility] = useState("block");
  const [apiKlicValue, setApiKlicValue] = useState("");
  const [apiKlicZmenaVisibility, setApiKlicZmenaVisibility] = useState("hidden");
  ...
  useEffect(() => {
    ...
    const apiValue = localStorage.getItem('api_klic');
    if (apiValue) {
      setApiKlicValue(apiValue);
      setApiKlicVisibility("hidden");
      setApiKlicZmenaVisibility("block");
    }
  }, []);

  const handleApiChange = (event: any) => {
    setApiKlicValue(event.target.value)
  };

  const handleApiZmena = () => {
    setApiKlicVisibility("block");
    setApiKlicZmenaVisibility("hidden");
  };

  const handleApiSkryt = () => {
    setApiKlicVisibility("hidden");
    setApiKlicZmenaVisibility("block");
  };

  const handleApiSave = () => {
    localStorage.setItem('api_klic', apiKlicValue);
      ...
      <div className="relative h-full w-full transition-width flex flex-col overflow-auto items-stretch flex-1">
        <div className="flex-1 overflow-auto bg-gray-800">
            <div className="flex flex-col justify-center items-center m-4">
              <img src={robotUrl} className="w-20" style={{filter: "drop-shadow(0px 0px 20px #2b84a7)"}}></img>
          ...
          <div className="">
            <div className="h-full flex flex-col items-center text-sm bg-gray-800">
                ...
                <div className={`flex flex-row gap-2 items-center mb-2 text-white font-bold ${apiKlicVisibility}`}>
                  ...
                  <input 
                    ...
                    placeholder="Google API klíč..."
                  />
                  <button
                    onClick={handleApiSave}
                    className="p-2 bg-yellow-600 border-1 rounded-md"
                  >Uložit</button>
                  <button
                    onClick={handleApiSkryt}
                    className="p-2 bg-gray-600 text-gray-900 border-1 rounded-md"
                  >Skrýt</button>
                </div>

                <div className={apiKlicZmenaVisibility}>
                  <button
                      onClick={handleApiZmena}
                      className="p-2 bg-yellow-600 border-1 rounded-md"
                    >Změnit API klíč</button>
                </div>
